#!/usr/bin/env bash
# =============================================================================
# DESEN ALTINDA A/B — desen SABİT, bir güdüm anahtarı DÖNÜŞÜMLÜ
#   araclar/desen_ab.sh <ad> <desen> <ENV_ADI> <deger0,deger1> <tekrar> <sure>
#
# Örn: araclar/desen_ab.sh OD_IZ daire DOW_IZ 0,1 4 150
#
# NEDEN AYRI BETİK: `desen_kampanya.sh` desen TÜRLERİ arasında dönüyor.
# Burada desen sabit tutulur (arızanın ölçüldüğü senaryo) ve güdüm
...
AD="${1:?kampanya adi}"; DES="${2:?desen turu}"; ENVAD="${3:?env adi}"
IFS=',' read -ra DEG <<< "${4:?degerler}"
TEKRAR="${5:-4}"; SURE="${6:-150}"

echo -n hibrit > .gudum_kipi
mkdir -p "logs/$AD"
echo "=== DESEN ALTINDA A/B: $AD ==="
echo "    desen sabit   : $DES"
...
PYEOF
    ...
    python3 araclar/desen.py "$DES" --ad "$AD/$ETIKET" \
