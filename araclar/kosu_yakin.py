# -*- coding: utf-8 -*-
"""YAKIN MENZİL ölçümü: uyarlanabilir çözünürlük EŞİĞİNİ belirlemek için.
Soru: kutu >40 px iken (menzil <25 m) 960 yeterli mi, yoksa 1920 mi şart?
Yöntem: hedefin 12-30 m bandında KAL, her karede iki çözünürlüğü de koş."""
...
try:
    ...
    ok2,R=ucus.yaklas(cev, 30.0, 240.0, log=lambda s: print(s,flush=True))
    print(f"yaklaşma {'OK' if ok2 else 'SÜRE'} R={R:.0f}", flush=True)
    if R>60: raise SystemExit("yakına girilemedi")
    f=open("logs/yakin_menzil.csv","w",newline=""); w=csv.writer(f)
    w.writerow(["menzil","bek_cx","bek_cy","bek_px","imgsz","kutu_sayisi",
                "hedef_bulundu","argmax_hedef","conf_hedef","hata_px"])
    n=0; t0=time.time()
    while time.time()-t0<230:
        # 12-30 m bandında TUT: yakınsa geri çekil, uzaksa yaklaş
        R,ez,hy,dz = ucus.istasyon(cev, 0.30 if R>26 else (-0.10 if R<15 else 0.05))
        if R>70: print("  hedef kaçtı, yeniden yaklaşılıyor", flush=True)
        ...
        for iz in (960,1920):
            ...
            bul=0;amx=0;cf=0.0;hp=-1
            for i,b in enumerate(bl):
                x1,y1,x2,y2=b.xyxy[0].tolist(); cx,cy=(x1+x2)/2,(y1+y2)/2
                if math.hypot(cx-bcx,cy-bcy)<=max(45,2.5*bp) and 0.4*bp<=(x2-x1)<=3.0*bp:
                    bul=1; cf=float(b.conf); hp=math.hypot(cx-bcx,cy-bcy)
                    if i==0: amx=1
                    break
            w.writerow([f"{R:.2f}",f"{bcx:.0f}",f"{bcy:.0f}",f"{bp:.1f}",iz,
                        len(bl),bul,amx,f"{cf:.3f}",f"{hp:.1f}"])
        n+=1
        if n%40==0: print(f"  n={n} R={R:.0f} kutu≈{bp:.0f}px", flush=True)
        time.sleep(0.003)
