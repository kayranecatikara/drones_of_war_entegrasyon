#!/usr/bin/env bash
# =============================================================================
# MANEVRA KAMPANYASI — yakında hafif, görsel fazda sert manevra
#   araclar/manevra_kampanya.sh <ad> <kip1,kip2> <tekrar> <sure_s>
# Örn: araclar/manevra_kampanya.sh KM1 yok,kademeli 4 150
#
# ⛔ `yok` kolu HER KAMPANYADA koşulur: manevrasız sonucu bilmeden
#    manevralı sonuç yorumlanamaz (§3.3).
# DÖNÜŞÜMLÜ (§4): her turda tüm kipler sırayla.
...
AD="${1:?kampanya adi}"; KIPLER="${2:?kipler}"
TEKRAR="${3:-4}"; SURE="${4:-150}"
echo -n hibrit > .gudum_kipi
mkdir -p "logs/$AD"
IFS=',' read -ra LISTE <<< "$KIPLER"
echo "=== MANEVRA KAMPANYASI $AD ==="
echo "    kipler   : ${LISTE[*]}"
...
PYEOF
    ...
    DOW_GORSEL=1 DOW_DET_GOSTER=1 DOW_KIP=hibrit \
      ...
      >> "logs/$AD.log" 2>&1 &
    KOSU_PID=$!
    python3 araclar/manevra.py "$D" --ad "$AD/$ETIKET" \
