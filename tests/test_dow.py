...
# ---------------------------------------------------------------- B61
def test_B61_hucum_hizi_tavani():
    """⭐ Ö-K · HÜCUM HIZI TAVANI — komut ile gerçekleşen arasındaki açık.

    ÖLÇÜM GEREKÇESİ (KH1, 20 Hz, GÖRSEL fazı, 4 uçuş):
        KOMUT 28.0 m/s (hep doymuş) · GERÇEKLEŞEN 22.1 · AÇIK 5.9
    İleri çubuk 0.26, doyum %1. Çeviricinin "dogru" modeliyle birebir:
        a = K_V·Δv = 1.5×5.9 = 8.85 ; çubuk = a/A_MAX = 8.85/34 = 0.26 ✓
    Hız döngüsü yalnız ORANSAL -> kalıcı hata kapanmıyor.

    Bekçi:
      1. VARSAYILAN V_HUCUM_UST == V_HUCUM -> kırpma BİT BİT aynı.
      2. Tavan yükselince komut edilen hız GERÇEKTEN yükselir.
      3. V_MIN alt sınırı korunur (tavan saçma küçük verilse bile).
      4. V_HUCUM (kabul edilmiş ayar) DEĞİŞMEDEN durur — B30 onu sınar.
    """
    from dow.gudum import ibvs
    C = ibvs.IbvsCfg
    eski = C.V_HUCUM_UST
    # kutu küçük -> hata_px dev -> v_istek tavana dayanır (ölçüldü: 40-150 px)
    ARG = (960.0, 540.0, 60.0, 48.0, 0.0, -2.0, 3.0)
    try:
        assert C.V_HUCUM == 28.0, "V_HUCUM kabul edilmiş ayar, değişmemeli"

        C.V_HUCUM_UST = C.V_HUCUM
        _, _, _, _, t0 = ibvs.komut(*ARG, 0.0, 0.05)
        assert abs(t0["ibvs_v"] - C.V_HUCUM) < 1e-9, \
            "varsayılan tavanda komut V_HUCUM olmalı (%.3f)" % t0["ibvs_v"]

        C.V_HUCUM_UST = 40.0
        _, _, _, _, t1 = ibvs.komut(*ARG, 0.0, 0.05)
        assert abs(t1["ibvs_v"] - 40.0) < 1e-9, \
            "tavan yükselince komut da yükselmeli (%.3f)" % t1["ibvs_v"]
        assert t1["ibvs_v"] > t0["ibvs_v"], "mekanizma yönü ters"

        # saçma küçük tavan V_MIN'in altına indirmemeli
        C.V_HUCUM_UST = 0.0
        _, _, _, _, t2 = ibvs.komut(*ARG, 0.0, 0.05)
        assert t2["ibvs_v"] >= C.V_MIN - 1e-9, \
            "tavan sıfır olsa bile V_MIN korunmalı (%.3f)" % t2["ibvs_v"]
    finally:
        C.V_HUCUM_UST = eski
